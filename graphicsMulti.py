...
MODIFIED TO KEEP TRACK OF MULTIPLE KEY PRESSES (line 230)
(source: http://stackoverflow.com/a/2141044)
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        """You may change or add to the array on line 235 to
        change or add to the keys you are keeping track of"""

        self.pressed = {}

        for char in ["w","s","o", "l"]:
            self.bind_all("<KeyPress-%s>" % char, self._pressed)
            self.bind_all("<KeyRelease-%s>" % char, self._released)
    ...
    # Modified to inclued new function which returns a boolean list
